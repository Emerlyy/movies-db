      ...
      <Paper sx={{ py: 6, flexGrow: 1 }}>
        <Container maxWidth="lg">
          <Box mb={4} sx={{ display: 'flex', justifyContent: 'space-between' }}>
            <TextField sx={{ width: '70%' }} size='small' color='text' autoComplete="off" value={query} onChange={(e) => handleQueryChange(e.target.value)} label='Search'></TextField>
            <ToggleButtonGroup
              value={type}
              size='small'
              exclusive
          ...
          </Box>
          {
            results.length ?
              <>
                ...
                <Pagination sx={{ marginX: 'auto', width: 'fit-content' }} siblingCount={3} count={Math.min(total_pages, 500)} variant="outlined" shape="rounded" page={page} onChange={handlePageChange} />
              </>
